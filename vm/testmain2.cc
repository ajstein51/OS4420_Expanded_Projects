...
// vars
int frameamount = 0, pageamount = 0, taken = 0, referenced = 0, recovered = 0, missed = 0;
bool debug = false;
vector<string> input;
vector<pages> page;
vector<frames> frame;
...
// g++ -Werror -Wall -std=c++11 -O3 main.cc
...
void lru(){
    ...
    // bandaid fix: check if anything in the pages have a num that isnt on the frame
    // for(int i = 0; i < pageamount; i++){
    //     if(page[i].framenum)
    //     int framenum = stoi(page[i].framenum);
    //     if(frame[framenum].obj != i)
    // }
    // print out stack
    if (debug == true)
    {
...
void randswap(){
    ...
    for (size_t i = 0; i < input.size(); i++){
        ...
        else
        {
            ...
            {
                page[index].type = "Mapped";
                page[index].framenum = to_string(i);
                frame[i].obj = index; // mark the page num
                frame[i].first_use = i, frame[i].last_use = i;
                frame[i].inuse = 1; // mark the frame inuse
            ...
            if(inflag == 0 && (signed)i >= frameamount){
                ...
                // counters
                frame[p.second].first_use = i + 1, frame[p.second].last_use = i + 1;
                referenced++;
                missed++;
                taken++;
                // frames recovered from disk, if we wrote to the page and then later rewrote/read to the page
